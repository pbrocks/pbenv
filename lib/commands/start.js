...
module.exports = async function start( { spinner, debug, update, xdebug } ) {
	...
	if ( ! config.detectedLocalConfig ) {
		...
		spinner.warn(
			`Warning: could not find a .pbrocks-env.json configuration file and could not determine if '${ configDirectoryPath }' is a WordPress installation, a plugin, or a theme.`
		);
		spinner.start();
	}
	...
	/**
	 * If the Docker image is already running and the `pbrocks-env` files have been
	...
	if ( shouldConfigureWp ) {
		...
		try {
			...
			// stop pbrocks-env from working correctly.
		}
