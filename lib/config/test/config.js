...
describe( 'readConfig', () => {
	...
	describe( 'config file', () => {
		it( 'should throw a validation error if config is invalid JSON', async () => {
			...
			expect.assertions( 2 );
			try {
				await readConfig( '.pbrocks-env.json' );
			} catch ( error ) {
				expect( error ).toBeInstanceOf( ValidationError );
				expect( error.message ).toContain( 'Invalid .pbrocks-env.json' );
			}
		} );
		...
		it( 'should throw a validation error if config cannot be read', async () => {
			...
			try {
				...
				expect( error.message ).toContain(
					'Could not read .pbrocks-env.json'
				);
			}
		} );
		...
		it( 'should infer a core config when ran from a core directory', async () => {
			...
			const config = await readConfig( '.pbrocks-env.json' );
		...
		it( 'should use the WP_ENV_HOME environment variable only if specified', async () => {
			...
			const configWith = await readConfig( '.pbrocks-env.json' );
			expect(
			...
			const configWithout = await readConfig( '.pbrocks-env.json' );
			expect(
		...
		it( 'should use a non-private folder with Snap-installed Docker', async () => {
			...
			const config = await readConfig( '.pbrocks-env.json' );
			expect( config.workDirectoryPath.includes( '.pbrocks-env' ) ).toBe(
				false
			);
			expect( config.workDirectoryPath.includes( 'pbrocks-env' ) ).toBe(
				true
		...
		it( 'should match snapshot', async () => {
			...
			// as well as the pbrocks-env directory which is unique on every machine.
	...
	describe( 'mappings parsing', () => {
		...
		it( 'throws an error if a mapping is badly formatted', async () => {
			...
			try {
				...
				expect( error ).toBeInstanceOf( ValidationError );
				expect( error.message ).toContain(
					'Invalid .pbrocks-env.json: "mappings.test" should be a string.'
				);
			}
		} );
		...
		it( 'throws an error if mappings is not an object', async () => {
			...
			try {
				...
				expect( error ).toBeInstanceOf( ValidationError );
				expect( error.message ).toContain(
					'Invalid .pbrocks-env.json: "mappings" must be an object.'
				);
			}
		} );
	...
	describe( 'wp config values', () => {
		...
		it( 'should override config values using the .override file first', async () => {
			readFile.mockImplementation( ( filename ) => {
				let result;
				if ( filename === '.pbrocks-env.json' ) {
					result = {
						config: {
							SCRIPT_DEBUG: '1',
						},
					};
				} else if ( filename === '.pbrocks-env.override.json' ) {
					result = {
						config: {
							SCRIPT_DEBUG: '2',
						},
...
async function testPortNumberValidation( portName, value, envText = '' ) {
	...
	try {
		...
		expect( error ).toBeInstanceOf( ValidationError );
		expect( error.message ).toContain(
			`Invalid .pbrocks-env.json: "${ envText }port" must be an integer.`
		);
	}
	jest.clearAllMocks();
}
