...
/**
 * pbrocks-env configuration.
 *
 * @typedef WPConfig
 * @property {string}                           name                    Name of the environment.
 * @property {string}                           configDirectoryPath     Path to the .pbrocks-env.json file.
 * @property {string}                           workDirectoryPath       Path to the work directory located in ~/.pbrocks-env.
 * @property {string}                           dockerComposeConfigPath Path to the docker-compose.yml file.
 * @property {boolean}                          detectedLocalConfig     If true, pbrocks-env detected local config and used it.
...
/**
 * Reads, parses, and validates the given .pbrocks-env.json file into a pbrocks-env config
 * object for internal use.
 *
 * @param {string} configPath Path to the .pbrocks-env.json file.
 *
 * @return {WPConfig} A parsed and validated pbrocks-env config object.
 */
module.exports = async function readConfig( configPath ) {
	...
	// The specified base configuration from .pbrocks-env.json or from the local
	// source type which was automatically detected.
	const baseConfig =
		( await readRawConfigFile( '.pbrocks-env.json', configPath ) ) ||
		( await getDefaultBaseConfig( configPath ) );

	// Overriden .pbrocks-env.json on a per-user case.
	const overrideConfig =
		( await readRawConfigFile(
			'.pbrocks-env.override.json',
			configPath.replace( /\.pbrocks-env\.json$/, '.pbrocks-env.override.json' )
		) ) || {};

	const detectedLocalConfig =
		Object.keys( { ...baseConfig, ...overrideConfig } ).length > 0;

	// Default configuration which is overridden by .pbrocks-env.json files.
	...
	if ( allPorts.size !== allEnvs.length ) {
		throw new ValidationError(
			'Invalid .pbrocks-env.json: Each port value must be unique.'
		);
	}
	...
	const mergeNestedObjs = ( key ) =>
		Object.assign(
			...
			...configs.map( ( config ) => {
				...
				throw new ValidationError(
					`Invalid .pbrocks-env.json: "${ key }" must be an object.`
				);
			} )
		);
...
/**
 * Detects basic config options to use if the .pbrocks-env.json config file does not
	...
	const updateEnvUrl = ( configKey ) => {
		[ 'development', 'tests' ].forEach( ( envKey ) => {
			try {
				...
				throw new ValidationError(
					`Invalid .pbrocks-env.json: config.${ configKey } must be a valid URL.`
				);
			}
		} );
	};
...
/**
 * Gets the `pbrocks-env` home directory in which generated files are created.
 *
 * By default: '~/.pbrocks-env/'. On Linux with snap packages: '~/pbrocks-env/'. Can be
 * overriden with the WP_ENV_HOME environment variable.
 *
 * @return {Promise<string>} The absolute path to the `pbrocks-env` home directory.
 */
