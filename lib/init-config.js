 ...
 * ./.pbrocks-env.json, creates ~/.pbrocks-env, ~/.pbrocks-env/docker-compose.yml, and
 * ~/.pbrocks-env/Dockerfile.
 *
...
module.exports = async function initConfig( {
	...
	const configPath = path.resolve( '.pbrocks-env.json' );
	 ...
	 * to the start command. For example, say you start pbrocks-env with Xdebug enabled.
	 ...
	 * arg. This basically makes it such that pbrocks-env start is the only command
	...
	if ( writeChanges ) {
		...
		spinner.fail(
			'pbrocks-env has not yet been initialized. Please run `pbrocks-env start` to install the WordPress instance before using any other commands. This is only necessary to set up the environment for the first time; it is typically not necessary for the instance to be running after that in order to use other commands.'
		);
		process.exit( 1 );
	}
...
/**
 * Generates the Dockerfile used by pbrocks-env's development instance.
 *
